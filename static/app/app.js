...
(function () {
  ...
  function go(tab) {
    ...
    try { renderCartBar(); } catch (e) {}       // keep the cart bar in sync on every view
  }
  ...
  function viewAccounts() {
    ...
    // Fire /capacity IN PARALLEL but do NOT block the screen on it — the pool poll can
    // take a few seconds. Render the buyer's own slots (fast DB read) immediately with a
    // "Refreshing…" placeholder, then fill the availability number + price chips when
    // /capacity resolves. Guarded by stale(seq) so a late reply can't clobber a newer view.
    var capP = api("/capacity").catch(function () { return null; });
    api("/slots").catch(function () { return { slots: [] }; })
      .then(function (r) {
        ...
        var html = '<section class="cap"><div class="cap-dot"></div><div class="cap-main"><div class="cap-lbl">'+esc(t("acc_pool"))+'</div>' +
          '<div class="cap-val" id="capVal">🔄 ' + esc(t("cap_refreshing")) + '</div></div>' + icon("shield", "cap-ic") + '</section>' +
          '<div class="prices" id="capPrices"></div>' +
          '<h2 class="sec">'+esc(t("acc_subs"))+'</h2>';
        if (!slots.length) {
        ...
        Array.prototype.forEach.call(document.querySelectorAll(".sub[data-username]"), function (card) {
          ...
          var activating = card.getAttribute("data-activating") === "1";
          var route = function () {
            if (activating) { toast(t("st_activating")); }
            else if (expired) { openRenew(uname); }
            else { go("manage"); }
          };
          card.addEventListener("click", route);
          card.addEventListener("keydown", function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); route(); } });
        });
        // Async fill of the availability number + price chips (only if still current).
        capP.then(function (cap) {
          if (stale(seq)) return;                       // a newer render superseded us → drop
          cap = cap || {};
          var cv = el("capVal");
          if (cv) cv.textContent = (cap.available != null ? cap.available : "—") + " " + t("slots_available");
          ...
          var cp = el("capPrices"); if (cp) cp.innerHTML = priceRow;
        });
      }).catch(function () { if (!stale(seq)) fatal(t("err_dashboard")); });
  }
  function subCard(s) {
    var isActivating = s.status === "activating" || s.activating;
    var isExp = !isActivating && (s.expired || isExpiredIso(s.expires_at));
    var online = s.online && !isExp && !isActivating;
    var sl = isActivating ? "" : secondLine(s);
    var statusTxt = isActivating ? t("st_activating")
      : (isExp ? t("st_expired") : (online ? t("st_online") : t("st_offline")));
    // The WHOLE card is the tap target (role=button); data-* let one handler route it:
    // active → Manage, expired → renew, activating → a "please wait" toast.
    return '<div class="sub tap' + (isActivating ? " activating" : "") + '" data-username="' + esc(s.stake_username || "") +
      '" data-expired="' + (isExp ? "1" : "0") + '" data-activating="' + (isActivating ? "1" : "0") + '" role="button" tabindex="0">' +
      '<div class="sub-top"><div class="sub-name">' + esc(s.stake_username || "—") + '</div>' +
      '<span class="dot ' + (online ? "on" : "off") + '"></span>' +
      '<span class="sub-status">' + esc(statusTxt) + '</span>' +
      ...
      (isActivating ? '' : '<span class="tag alt" data-exp="' + esc(s.expires_at || "") + '">' + esc(fmtExpiry(s.expires_at)) + '</span>') +
  ...
  function renderBuyStep2() {
    ...
    Array.prototype.forEach.call(rest.querySelectorAll(".plan"), function (b) {
      b.addEventListener("click", function () {
        ...
        renderBuyStep3();
        // Nudge the buyer down to the "Configure this slot" step that just appeared.
        setTimeout(function () {
          try { var e = el("buyStep3"); if (e) e.scrollIntoView({ behavior: "smooth", block: "start" }); } catch (e) {}
        }, 120);
      });
    });
  }
  ...
  // mode 'pay' (renewal → pay this one slot now) vs default 'cart' (buy → add to cart).
  function renderBuyStep3(mode) {
    var isPay = (mode === "pay");
    var s3 = el("buyStep3");
    var label = isPay
      ? (t("buy_pay") + (buyState.plan ? " · $" + buyState.plan.price_usd : ""))
      : (t("add_to_cart") + (buyState.plan ? " · $" + buyState.plan.price_usd : ""));
    s3.innerHTML = '<label class="lab">3 · '+esc(t("buy_step3"))+'</label><div class="card">' +
      cfgFields({}) +
      '<button class="btn primary" id="payBtn" type="button">'+esc(label)+'</button></div>';
    wireToggles(s3);   // FIX: Buy-flow toggles were never wired (Manage/Drop were)
    el("payBtn").addEventListener("click", isPay ? doBuyPay : addToCart);
  }
  ...
  function doBuyPay() {
    ...
    api("/order/begin", { method: "POST", body: { plan_code: buyState.plan.code, token: buyState.token, config: cfg } })
      .then(function (d) {
        ...
        // Order is placed; close the renewal sheet if it was the entry point (no-op
        // in the Buy tab where no sheet is open) so the pay/poll view isn't covered.
        closeSheet();
  ...
  // ── Renewal (tapping an EXPIRED account on the Accounts page) ──────────────
  // Reuses the Buy pieces. The slot's token was wiped on expiry, so the buyer
  // re-enters it: valid → renewal plans + config + pay (a fresh allocation for the
  // same account); invalid → a popup to buy a new slot with a new token.
  function openRenew(username) {
    api("/capacity").then(function (cap) {
      buyState = { token: null, username: username || null, plan: null, plans: cap.plans || [] };
      var body =
        '<div class="card"><label class="lab">' + esc(t("renew_token_ph")) + '</label>' +
        '<input class="inp" id="renewKey" type="password" placeholder="' + esc(t("renew_token_ph")) + '" autocomplete="off">' +
        '<button class="btn" id="renewVerify" type="button">' + esc(t("renew_verify")) + '</button>' +
        '<div class="verify-out" id="renewOut"></div></div>' +
        '<div id="renewRest"></div>';
      openSheet(t("renew_title") + " " + (username || ""), body);
      el("renewVerify").addEventListener("click", doRenewVerify);
    }).catch(function () { toast(t("err_plans"), true); });
  }
  function doRenewVerify() {
    var key = (el("renewKey").value || "").trim();
    var out = el("renewOut");
    if (!key) { out.textContent = t("verify_enter_key"); out.className = "verify-out bad"; return; }
    out.textContent = t("verifying"); out.className = "verify-out";
    el("renewVerify").disabled = true;
    api("/verify-token", { method: "POST", body: { token: key } }).then(function (d) {
      el("renewVerify").disabled = false;
      if (d.valid) {
        buyState.token = key; buyState.username = d.username || buyState.username;
        out.innerHTML = icon("check") + " " + t("verified") + ": <b>" + esc(d.username || "account") + "</b>";
        out.className = "verify-out good"; haptic("success");
        renderRenewPlans();
      } else {
        haptic("error"); showRenewInvalid();
      }
    }).catch(function () { el("renewVerify").disabled = false; out.textContent = t("verify_failed"); out.className = "verify-out bad"; });
  }
  function renderRenewPlans() {
    var rest = el("renewRest");
    rest.innerHTML = '<label class="lab">' + esc(t("renew_choose_plan")) + '</label><div class="plans">' + planTiles(buyState.plans) + '</div>' +
      '<div id="buyStep3"></div>';
    // planTiles does NOT self-wire — replicate renderBuyStep2's plan-click wiring.
    Array.prototype.forEach.call(rest.querySelectorAll(".plan"), function (b) {
      b.addEventListener("click", function () {
        Array.prototype.forEach.call(rest.querySelectorAll(".plan"), function (x) { x.classList.remove("sel"); });
        b.classList.add("sel");
        buyState.plan = buyState.plans[Number(b.getAttribute("data-i"))];
        renderBuyStep3("pay");   // renewal pays this ONE slot now (not the cart)
      });
    });
  }
  function showRenewInvalid() {
    var body = '<div class="empty"><div class="empty-t">' + esc(t("renew_invalid_title")) + '</div>' +
      '<div class="empty-s">' + esc(t("renew_invalid_body")) + '</div>' +
      '<button class="btn primary" id="renewBuyNew" type="button">' + esc(t("renew_buy_new")) + '</button></div>';
    openSheet(t("renew_invalid_title"), body);
    var b = el("renewBuyNew"); if (b) b.addEventListener("click", function () { closeSheet(); go("buy"); });
  }

  // ── Cart (multi-slot, one combined payment) ────────────────────────────────
  var CART_KEY = "scc_cart";
  function cartLoad() { try { return JSON.parse(localStorage.getItem(CART_KEY) || "[]") || []; } catch (e) { return []; } }
  function cartSave(c) { try { localStorage.setItem(CART_KEY, JSON.stringify(c)); } catch (e) {} renderCartBar(); }
  function cartAdd(item) { var c = cartLoad(); c.push(item); cartSave(c); }
  function cartRemove(id) { cartSave(cartLoad().filter(function (x) { return x.id !== id; })); }
  function cartClear() { cartSave([]); }
  function cartCount() { return cartLoad().length; }
  function cartTotal() { return cartLoad().reduce(function (s, x) { return s + (x.plan && Number(x.plan.price_usd) || 0); }, 0); }
  function newItemId() { try { return crypto.randomUUID(); } catch (e) { return String(Date.now()) + Math.random().toString(16).slice(2); } }

  function addToCart() {
    if (!buyState.token || !buyState.plan) { toast(t("pay_need"), true); return; }
    cartAdd({ id: newItemId(), token: buyState.token, username: buyState.username,
              plan: { code: buyState.plan.code, label: buyState.plan.label, price_usd: buyState.plan.price_usd },
              config: readCfg() });
    haptic("success"); toast(t("added_to_cart"));
    viewBuy();                                  // reset to step 1 so they can add another
  }

  // The cart bar lives in the persistent #ctabar (above the nav) — visible on any tab.
  function renderCartBar() {
    var bar = el("ctabar"); if (!bar) return;
    var n = cartCount();
    if (!n) { bar.className = "ctabar hidden"; bar.innerHTML = ""; return; }
    bar.className = "ctabar";
    bar.innerHTML = '<button class="cta-cart" id="cartBarBtn" type="button">' +
      '<span>🛒 ' + n + ' · $' + esc(cartTotal()) + '</span>' +
      '<span class="cta-go">' + esc(t("view_cart")) + '</span></button>';
    var b = el("cartBarBtn"); if (b) b.addEventListener("click", openCartSheet);
  }

  function openCartSheet() {
    var c = cartLoad();
    if (!c.length) { openSheet(t("cart_title"), '<div class="empty-s pad">' + esc(t("cart_empty")) + '</div>'); return; }
    var rows = c.map(function (x) {
      return '<div class="citem"><div class="ci-main"><div class="ci-name">' + esc(x.username || "—") + '</div>' +
        '<div class="ci-plan">' + esc(x.plan && x.plan.label || "") + ' · $' + esc(x.plan && x.plan.price_usd || 0) + '</div></div>' +
        '<button class="ci-x" data-id="' + esc(x.id) + '" type="button" aria-label="' + esc(t("cart_remove")) + '">✕</button></div>';
    }).join("");
    var body = '<div class="cart-list">' + rows + '</div>' +
      '<div class="cart-foot"><span>' + esc(t("cart_total")) + '</span><b>$' + esc(cartTotal()) + '</b></div>' +
      '<button class="btn primary" id="cartPayBtn" type="button">' + esc(t("cart_pay")) + ' · $' + esc(cartTotal()) + '</button>';
    openSheet(t("cart_title"), body);
    Array.prototype.forEach.call(document.querySelectorAll(".ci-x"), function (b) {
      b.addEventListener("click", function () { cartRemove(b.getAttribute("data-id")); openCartSheet(); });
    });
    el("cartPayBtn").addEventListener("click", doCartPay);
  }

  function doCartPay() {
    var c = cartLoad();
    if (!c.length) { return; }
    var items = c.map(function (x) { return { token: x.token, plan_code: x.plan.code, config: x.config }; });
    var btn = el("cartPayBtn"); if (btn) { btn.disabled = true; btn.textContent = t("creating_order"); }
    api("/order/cart-begin", { method: "POST", body: { items: items } })
      .then(function (d) {
        return api("/order/cart-pay", { method: "POST", body: { cart_id: d.cart_id } })
          .then(function (p) { return { cart_id: d.cart_id, pay: p }; });
      }).then(function (r) {
        closeSheet();
        cartClear();                            // orders now live server-side
        if (r.pay.already) { toast(t("already_active")); go("accounts"); return; }
        openPayUrl(r.pay.pay_url);
        pollCart(r.cart_id, r.pay.pay_url);
      }).catch(function (e) {
        if (btn) { btn.disabled = false; btn.textContent = t("cart_pay"); }
        toast(e && e.code === "verify_invalid" ? t("verify_invalid") : t("pay_failed"), true);
      });
  }

  function _payWaitView(payUrl) {
  ...
  function pollCart(cartId, payUrl) {
    if (!payUrl) {
    ...
    var iv = setInterval(function () {
      ...
      tries++;
      api("/order/cart/" + cartId).then(function (d) {
        if (d && d.settled) {                   // payment processed (some may be Activating)
          clearInterval(iv); haptic("success");
          var msg = (d.activating > 0)
            ? (d.allocated + " " + t("st_active") + " · " + d.activating + " " + t("st_activating"))
            : t("sub_active_s");
          view('<div class="empty">' + icon("check", "empty-ic") + '<div class="empty-t">'+esc(t("sub_active_t"))+'</div>' +
            '<div class="empty-s">' + esc(msg) + '</div>' +
  ...
  function renderStats(win) {
    ...
    api("/stats?window=" + win + "&type=" + statsType).then(function (d) {
      ...
      var byUserRows = Object.keys(byUser).map(function (u) {
        ...
        var amts = Object.keys(amounts).map(function (c) { return esc(amounts[c]) + " " + esc(c.toUpperCase()); }).join(", ");
        var right = esc(info.claims || 0) + " " + esc(t("codes_word")) + (amts ? " · " + amts : "");
        return '<div class="euser"><span class="euser-name">' + esc(u) + '</span><span class="euser-amt">' + right + '</span></div>';
      }).join("");
  ...
  function boot() {
    ...
    initScrollCue();
    renderNav(); markNav();
    try { renderCartBar(); } catch (e) {}       // restore the cart bar from localStorage
